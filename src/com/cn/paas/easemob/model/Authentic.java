...
public class Authentic {
	...
	private void flush() {
		synchronized (Authentic.class) {
			try {
				if (token.isExpire()) {
					if (_Global.APP_CLIENT_ID != null
							&& _Global.APP_CLIENT_SECRET != null) {
						Map<String, Object> param = new HashMap<String, Object>();
						param.put("grant_type", "client_credentials");
						param.put("client_id", _Global.APP_CLIENT_ID);
						param.put("client_secret", _Global.APP_CLIENT_SECRET);
						TalkNode res = service.request(_Global.URR_TOKEN,
								_Global.HTTP_METHOD_POST, param, null, null);
						if (res != null && res.getAccess_token() != null
								&& res.getExpires_in() != null
								&& res.getExpires_in() > 0) {
							token = new Token(res.getAccess_token(),
									res.getExpires_in() * 1000
